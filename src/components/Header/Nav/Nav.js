...
const calculateNavItemsHeight = (navDOMElements) =>
  [...navDOMElements].reduce(
    (currentHeight, el) => el.getBoundingClientRect().height + currentHeight,
    0,
  );
...
export const Nav = ({ categories, setCategories }) => {
  ...
  return (
    <nav className={styles.navbar} role="navigation">
      ...
      <ul className={styles.nav} ref={navRef} id="mainNavigation">
        {isLoadingCategories ? (
          <span role="status">Loading menu items...</span>
        ) : (
          <>
